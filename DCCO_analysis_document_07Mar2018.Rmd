...
knitr::include_graphics(path="fig.North Bay.gam.NoError.png")
knitr::include_graphics(path="fig.North Bay.gam.png")
figure.num<-figure.num+1
```
Fig `r figure.num`. Colony counts (top), GAM colony trends with SE (middle), and GAM regional trend with (middle) and without SD (bottom) for North Bay region. Note that the trend estimates are on a different scale from the counts, so the trend estimates cannot be used to visually estimate count predictions.
...
figure.num<-figure.num+1
```
Fig `r figure.num`. GAM trend for all SF Bay colonies. Note that the trend estimates are on a different scale from the counts, so the trend estimates cannot be used to visually estimate count predictions.
